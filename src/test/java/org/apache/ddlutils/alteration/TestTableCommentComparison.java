...
public class TestTableCommentComparison extends TestComparisonBase {
    ...
    public void testAddColumn()
    {
        ...
        final String MODEL2 =
                "<?xml version='1.0' encoding='ISO-8859-1'?>\n" +
                        "<database xmlns='" + DatabaseIO.DDLUTILS_NAMESPACE + "' name='test'>\n" +
                        "  <table name='TableA' description='description example'>\n" +
                        "    <column name='ColPK' type='INTEGER' primaryKey='true' required='true'/>\n" +
                        "  </table>\n" +
                        "</database>";
        ...
        TableCommentChange change = (TableCommentChange)changes.get(0);

        assertEquals("TableA",
                change.getModifiedTable().getName());
    }
